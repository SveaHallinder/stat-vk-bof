...
export const ResetPasswordPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validera lösenord
    try {
      schemas.passwordReset.parse({ password, confirmPassword });
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        enhancedToast.success('Lösenord återställt! Du kan nu logga in.');
        navigate('/login');
      } else {
        const errorData = await response.json().catch(() => ({}));
        enhancedToast.error(errorData.message || 'Kunde inte återställa lösenord');
      }
    } catch {
    ...
    <div className="min-h-screen bg-[#f5f7fa] flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl text-gray-900">Spara nytt lösenord</CardTitle>
          <CardDescription className="text-gray-600">Ange och bekräfta ditt nya lösenord</CardDescription>
        </CardHeader>
        <CardContent>
