...
- Verksamhetsdata (customers, cases, shifts, efforts, handlers):
  ...
  - Behandlare/insatser: avaktiveras; referenser i ärenden/tider bevaras.
...
30 2 * * * psql "$DATABASE_URL" -c "SELECT cleanup_old_audit_logs();" >/var/log/vk_audit_cleanup.log 2>&1
```
